      ...
      <section className="relative isolate flex min-h-screen items-center overflow-hidden">
        ...
        {!copy ? (
          ...
          <>
            ...
            <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pt-24 pb-12">
              {/* Testata */}
              <div className="text-center">
                ...
                <h1 className="mt-5 font-serif text-4xl md:text-6xl font-normal leading-none tracking-[-0.015em] text-white">
                  {lang === 'it' ? copy.page_title_it : copy.page_title_en}
                </h1>
                <p className="mt-3 font-serif text-lg md:text-xl text-white/90">
                  {lang === 'it' ? copy.subtitle_it : copy.subtitle_en}
                </p>
                <p className="mt-3 text-sm md:text-base text-white/70">
                  {lang === 'it' ? copy.intro_it : copy.intro_en}
                </p>
                <span aria-hidden="true" className="mx-auto mt-6 block h-px w-16 bg-[#C9BEA8]/60" />
              </div>

              {/* I quattro modi per parlarci */}
              <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-4">
                {/* Telefono */}
                <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-4 text-center backdrop-blur-[2px]">
                  <span className="mx-auto flex h-9 w-9 items-center justify-center rounded-full border border-[#C9BEA8]/50">
                    <svg className="h-4 w-4 text-[#C9BEA8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.3}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                  </span>
                  <h2 className="mt-3 font-serif text-sm text-white">{lang === 'it' ? copy.phone_label_it : copy.phone_label_en}</h2>
                  <a
                    href={copy.phone_tel_url}
                    onClick={() => trackPhoneCall('contact_page')}
                    className="mt-1 block text-sm text-white transition-colors hover:text-[#E8DFCC]"
                  >
                    {copy.phone_display}
                  </a>
                ...
                {/* WhatsApp */}
                <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-4 text-center backdrop-blur-[2px]">
                  <span className="mx-auto flex h-9 w-9 items-center justify-center rounded-full border border-[#25D366]/60">
                  ...
                  <h2 className="mt-3 font-serif text-sm text-white">{lang === 'it' ? copy.whatsapp_label_it : copy.whatsapp_label_en}</h2>
                  <a
                    href={copy.whatsapp_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 inline-flex items-center justify-center gap-1.5 border border-white/25 px-3 py-1.5 text-[11px] text-white transition-colors hover:bg-white hover:text-black"
                  >
                ...
                <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-4 text-center backdrop-blur-[2px]">
                  ...
                  <h2 className="mt-3 font-serif text-sm text-white">{lang === 'it' ? copy.email_label_it : copy.email_label_en}</h2>
                  <a href={`mailto:${copy.email_address}`} className="mt-1 block break-all text-[11px] leading-tight text-white transition-colors hover:text-[#E8DFCC]">
                    {copy.email_address}
                  </a>
                  <p className="mt-2 text-[8px] uppercase tracking-[0.18em] text-white/45">
                ...
                <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-4 text-center backdrop-blur-[2px]">
                  ...
                  <h2 className="mt-3 font-serif text-sm text-white">{lang === 'it' ? copy.hours_label_it : copy.hours_label_en}</h2>
                  <div className="mt-1 space-y-0.5 text-[11px] text-white/80">
              ...
              {/* Dove siamo */}
              <div className="mt-10 flex flex-col items-center justify-center gap-6 text-center md:flex-row md:gap-10 md:text-left">
