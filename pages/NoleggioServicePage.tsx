 ...
 * "Prenota Ora" che apre WhatsApp con l'operatore per concordare disponibilita'
 * e prenotazione (no booking/pagamento online).
 */
    ...
    <div className="bg-black text-white min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-28 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map(item => {
            const message =
              `Ciao DR7, vorrei prenotare ${asset}: ${item.name}. ` +
              `Potete dirmi disponibilità e dettagli?`;
              ...
              <div key={item.id} className="group rounded-2xl overflow-hidden bg-white/[0.03] border border-white/10 hover:border-dr7-gold/50 transition-colors flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-dr7-gold font-semibold whitespace-nowrap">{eur(item.price_per_day)}<span className="text-xs text-gray-400">/giorno</span></div>
                  </div>
                  {item.capacity != null && (
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-5 inline-flex items-center justify-center gap-2 w-full px-4 py-3 rounded-full bg-dr7-gold text-black font-semibold hover:opacity-90 transition-opacity"
                  >
        ...
        <p className="mt-10 text-center text-xs text-gray-500">
          Disponibilità e prenotazione si concordano direttamente con il nostro operatore via WhatsApp.
        </p>
      </div>
    </div>
  );
